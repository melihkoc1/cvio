...
const stepKeys = [
  { titleKey: 'step.0.title', subtitleKey: 'step.0.subtitle', icon: User, color: 'blue' },
  { titleKey: 'step.1.title', subtitleKey: 'step.1.subtitle', icon: Briefcase, color: 'indigo' },
  { titleKey: 'step.2.title', subtitleKey: 'step.2.subtitle', icon: GraduationCap, color: 'violet' },
  { titleKey: 'step.3.title', subtitleKey: 'step.3.subtitle', icon: Sparkles, color: 'purple' },
  { titleKey: 'step.4.title', subtitleKey: 'step.4.subtitle', icon: Award, color: 'pink' },
  { titleKey: 'step.5.title', subtitleKey: 'step.5.subtitle', icon: Settings, color: 'rose' },
] as const;
...
function LivePreviewPanel({ cvData }: { cvData: CVData }) {
  ...
  return (
    <>
      ...
      >
        <Eye className="w-4 h-4" /> {showMobile ? t('form.hidePreview') : t('form.preview')}
      </button>
...
export function AppForm() {
  ...
  const { cvData, updatePersonalInfo, addExperience, updateExperience, removeExperience,
    ...
    setCurrentPage, generateCV, user, setShowUpgradeModal, t } = app;
  ...
  const validateStep = () => {
    ...
    if (step === 0) {
      if (!cvData.personalInfo.fullName.trim()) newErrors.fullName = t('error.fullName');
      if (!cvData.personalInfo.email.trim()) newErrors.email = t('error.email');
      else if (!/\S+@\S+\.\S+/.test(cvData.personalInfo.email)) newErrors.email = t('error.emailInvalid');
    }
    setErrors(newErrors);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 pt-20 pb-16">
      ...
      <div>
        ...
        <div className="text-center mb-8 pt-6">
          ...
          <p className="text-gray-500 text-sm mt-1">{t('form.step')} {step + 1}/6 — {t(stepKeys[step].subtitleKey)}</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-5">
          ...
          <div className="flex justify-between mt-1.5">
            <span className="text-[10px] text-gray-400">{t('form.start')}</span>
            <span className="text-[10px] text-blue-600 font-semibold">{completionPct}{t('form.complete')}</span>
            <span className="text-[10px] text-gray-400">{t('form.done')}</span>
          </div>
        </div>
        ...
        <div className={`bg-white rounded-2xl border border-gray-100 shadow-sm transition-all duration-200 ${animating ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
          ...
          <div className="px-6 py-5 border-b border-gray-50">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                ...
                {(() => { const Icon = stepKeys[step].icon; return <Icon className="w-5 h-5" />; })()}
              </div>
              <div>
                <h2 className="text-lg font-bold text-gray-900">{t(stepKeys[step].titleKey)}</h2>
                <p className="text-xs text-gray-400">{t(stepKeys[step].subtitleKey)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            {step === 0 && (
              <div className="space-y-5">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <FormField label={t('field.fullName')} required>
                    <StyledInput
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <FormField label={t('field.phone')} hint={t('field.optional')}>
                    <StyledInput
                  ...
                  <FormField label={t('field.linkedin')} hint={t('field.optional')}>
                ...
                <div className="grid sm:grid-cols-2 gap-4"> 
                  <FormField label={t('field.github')} hint={t('field.optional')}>
                    <div className="relative">
                ...
                <FormField label={t('field.personalStatement')} hint={t('field.optional')}>
                  <StyledTextarea
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-start gap-3 bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                  ...
                  <div className="text-sm text-indigo-700 space-y-1">
                    <p className="font-semibold">{t('exp.banner.title')}</p>
                    <p className="text-indigo-600 text-xs leading-relaxed">{t('exp.banner.desc')}</p>
                  </div>
                </div>

                {cvData.experience.length === 0 && (
                  <EmptyState icon={Briefcase} title={t('exp.empty.title')} desc={t('exp.empty.desc')} />
                )}
                {cvData.experience.map((exp, i) => {
                  ...
                  <div key={exp.id} className="border border-gray-200 rounded-2xl overflow-hidden group hover:border-indigo-200 transition-colors">
                    ...
                    <div className="p-5 space-y-4">
                      {/* Type selector */}
                      <div>
                        <p className="text-xs font-semibold text-gray-500 mb-2">{t('exp.type')}</p>
                        <div className="flex gap-2 flex-wrap">
                          {(['work', 'project', 'volunteer', 'freelance'] as const).map(expT => (
                            <button
                              key={expT}
                              onClick={() => updateExperience(exp.id, { type: expT })}
                            ...
                            >
                              {expT === 'work' ? t('exp.work') : expT === 'project' ? t('exp.project') : expT === 'volunteer' ? t('exp.volunteer') : t('exp.freelance')}
                            </button>
                          ))}
                        </div>
                      </div>
                      ...
                      <FormField label={t('exp.whatDidYouDo')}>
                        <StyledTextarea
            ...
            {step === 2 && (
              <div className="space-y-5">
                ...
                {cvData.education.length === 0 && (
                  <EmptyState icon={GraduationCap} title={t('edu.empty.title')} desc={t('edu.empty.desc')} />
                )}
                ...
                >
                  <Plus className="w-4 h-4" /> {t('form.addEducation')}
                </button>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-5">
                ...
                {/* Input */}
                <FormField label={t('skills.add')} hint={t('skills.addHint')}>
                  <div className="relative">
                    <input
                ...
                {cvData.skills.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">{t('skills.added')} ({cvData.skills.length})</p>
                ...
                <div className="border border-gray-100 rounded-xl overflow-hidden">
                  <div className="flex border-b border-gray-100">
                    {[t('skills.technical'), t('skills.soft'), t('skills.industry')].map((cat, i) => (
                      <button
                        key={i}
            ...
            {step === 4 && (
              <div className="space-y-5">
                ...
                <div className="flex items-start gap-3 bg-pink-50 rounded-xl p-4 border border-pink-100">
                  ...
                  <p className="text-sm text-pink-700">{t('cert.optional')}</p>
                </div>
                {cvData.certificates.length === 0 && (
                  <EmptyState icon={Award} title={t('cert.empty.title')} desc={t('cert.empty.desc')} />
                )}
                ...
                >
                  <Plus className="w-4 h-4" /> {t('form.addCertificate')}
                </button>
              </div>
            )}
            ...
            {step === 5 && (
              <div className="space-y-6">
                {/* Language */}
                <FormField label={t('pref.cvLanguage')}>
                ...
                {/* Template */}
                <FormField label={t('pref.template')}>
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    {templateInfo.map(tmpl => (
                      <button
                        key={tmpl.id}
                        onClick={() => {
                          if ((tmpl.id === 'creative' || tmpl.id === 'professional' || tmpl.id === 'minimal') && user.plan === 'free') {
                            setShowUpgradeModal(true);
                            return;
                          }
                          updatePreferences({ template: tmpl.id as any });
                        }}
                        className={`rounded-2xl border-2 p-2 transition-all cursor-pointer text-left relative overflow-hidden ${
                          cvData.preferences.template === tmpl.id
                            ? 'border-blue-500 shadow-md ring-2 ring-blue-100'
                            : 'border-gray-200 hover:border-gray-300'
                        } ${(tmpl.id === 'creative' || tmpl.id === 'professional' || tmpl.id === 'minimal') && user.plan === 'free' ? 'opacity-70' : ''}`}
                      >
                        {(tmpl.id === 'creative' || tmpl.id === 'professional' || tmpl.id === 'minimal') && user.plan === 'free' && (
                        ...
                        <div className="aspect-[3/4] rounded-xl overflow-hidden mb-2 shadow-sm border border-gray-100">
                          <TemplateMiniPreview id={tmpl.id} />
                        </div>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-5 border border-blue-100">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-sm font-bold text-blue-900">{t('pref.ready')}</p>
                  </div>
                  <p className="text-sm text-blue-700 mb-4">{t('pref.readyDesc')}</p>
                  <div className="grid grid-cols-2 gap-3">
                    <button
          ...
          {step < 5 && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-50 bg-gray-50/50 rounded-b-2xl">
              ...
              >
                <ChevronLeft className="w-4 h-4" /> {t('form.back')}
              </button>
              ...
              >
                {step === 4 ? t('form.lastStep') : t('form.next')} <ChevronRight className="w-4 h-4" />
              </button>
            </div>
          )}
        </div>
